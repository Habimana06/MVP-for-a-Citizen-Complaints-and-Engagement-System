...
function AdminDashboard() {
  ...
  const fetchComplaints = async () => {
    try {
      ...
      const validComplaints = data.map(complaint => ({
        ...complaint,
        id: complaint._id || complaint.id, // Ensure we have a consistent id field
        _id: complaint._id || complaint.id // Keep _id for MongoDB compatibility
      })).filter(complaint => complaint.id); // Filter out any complaints without an ID
  ...
  const handleResponseSubmit = async (e) => {
    ...
    try {
      ...
      await complaintAPI.addResponse(selectedComplaint.id, response);
      await fetchComplaints();
      setResponse('');
